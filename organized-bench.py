...
def update_analysis(w, t, b_option, bs, avg_throughput, var_throughput, error_rate):
    ...
    while (workgroups * threads <= max_size):
        ...
        if not (power_of_two * workgroups * threads >= max_size):
            ...
            for b_option in ['a', 'c']:
                ...
                alt = 1
                for _ in range(n):
                    command = f"{executable} -d {device} -w {workgroups} -t {threads} -p {p} -b '{b_option}' -a '{alt}' -s'{power_of_two}'"
                ...
                update_analysis(workgroups, threads, b_option, power_of_two, avg_throughput, var_throughput, error_rate)
        ...
        if threads < max_threads:
            ...
            threads = 32  # Reset threads to initial value
            workgroups *= 2
...
for w_times_t_bs, b_data in analysis.items():
    ...
    max_throughput_results.append((w_times_t_bs, best_data['avg_throughput'], best_data['b_option'], math.sqrt(best_data['var_throughput']), best_data['w'], best_data['t'], best_data['bs']))
...
ax1.errorbar(w_times_t_bs_values, throughput_values, yerr=std_values, capsize=5, marker='o', linestyle='-', color=color, label="NVidia GTX 4070")
...
fig.savefig("exhaustive-bench.png")

with open('exhaustive-bench.pkl', 'wb') as f:
